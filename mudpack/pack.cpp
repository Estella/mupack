...
void functions(pointers *p, PE *pe)
{
	...
	psize = sfunc[0] + sfunc[1] + sfunc[2] + sizeof(pointers) + pe->scomparray+ pe->sdllimports + pe->sdllexports+sizeof(IMAGE_TLS_DIRECTORY32) + 12;
...
void construct(pointers *pt, PE *pe, DWORD sfunc[3], int section_size)
{
	...
	Assembler assembl;
	memset(&pt->opcode,0x90,sizeof(pt->opcode));
	...
	size_t code_size = assembl.getCodeSize();
	void *code_data = assembl.make();
	memcpy(&pt->opcode,code_data, code_size); 
	...
	DWORD ptls = pexports+ pe->sdllexports + 12;
	DWORD tlsrva = pexportsrva + pe->sdllexports + 12;
	memcpy((IMAGE_TLS_DIRECTORY32*)ptls,pImgTlsDir,sizeof(IMAGE_TLS_DIRECTORY32));
	if(pe->int_headers.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_TLS].Size)pe->int_headers.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_TLS].VirtualAddress = tlsrva;
	free(pImgTlsDir);
